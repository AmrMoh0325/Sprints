...
/************************************************************************************
* Parameters (in): uint8_t* pu8String, uint8_t u8bufferMaxSizeuint8_t* pu8String
* Parameters (out): enuErrorStatus_t
* Return value: 1=SUCCESS or 0=FAIL
...
enuErrorStatus_t UART_ReceiveHex(uint32_t * pu32Data)
{
   ...
   uint32_t result=0;
   //receive the number
   UART_ReceiveString(buffer,8);
   //loop on the received buffer
   for(uint8_t u8i=0;buffer[u8i];u8i++)
   {
      //if the character is between 0 and 9
      if (buffer[u8i]>='0' && buffer[u8i]<='9')
      {
         //shift the result buffer
         result<<=4;
         //add the number equivalent to the result buffer
         result+= (buffer[u8i]-'0');
      }
      //if the character is between A and F
      else if (buffer[u8i]>='A' && buffer[u8i]<='F')
      {
         //shift the result buffer
         result<<=4;
         //add the number equivalent to the result buffer
         result+= (buffer[u8i]-'A'+10);
      }
      //if any other number is encountered return error
      else
      {
         return ERROR;
      }
   }
   //store the result buffer in the sent buffer
   *pu32Data= result;
